...
Similar to `convert_frames_to_video`, this extracts the frames from a `.tar` 
file generated by the meshcat visualizer's record function and compiles them 
into a GIF using ImageMagick's `convert` command.

Note: This requires ImageMagick to be installed on your system.
"""
function convert_frames_to_gif(tar_file_path::AbstractString, output_path::AbstractString="output.gif"; framerate=30, overwrite=false)
    output_path = abspath(output_path)
    if !isfile(tar_file_path)
        error("Could not find the input file $tar_file_path")
    end
    ...
    mktempdir() do tmpdir
        ...
        # ImageMagick uses 'delay' in 1/100ths of a second.
        # So 30 fps ≈ 3.33 delay. We round to the nearest integer.
        delay = round(Int, 100 / framerate)
        
        cd(tmpdir) do
            # Grab all the PNG frames in the correct sequential order
            frames = sort(filter(f -> endswith(f, ".png"), readdir()))
            if isempty(frames)
                error("No PNG frames found in the extracted tar archive.")
            end
            
            # Construct and execute the ImageMagick command
            cmd = Cmd(["convert", "-delay", string(delay), "-loop", "0", frames..., output_path])
            try
                run(cmd)
            catch e
                error("Failed to create GIF. Please ensure ImageMagick is installed and accessible via the 'convert' command. Original error: $e")
            end
        end
    end
    @info("Saved output as $output_path")
